...
try {
  ...
  report.visualContracts = await evaluate("(() => { const fonts = { instrumentSerif: document.fonts.check('42px \\\"Instrument Serif\\\"'), inter: document.fonts.check('16px Inter'), ibmPlexMono: document.fonts.check('11px \\\"IBM Plex Mono\\\"') }; const motion = { gsap: document.documentElement.dataset.gsap === 'active', scrollTrigger: Boolean(document.querySelector('.pin-spacer')) || window.scrollY === 0 }; const staticA2Blocks = document.querySelectorAll('.manifesto__material img, .closing__material img').length; return { ok: Object.values(fonts).every(Boolean) && motion.gsap && motion.scrollTrigger && staticA2Blocks === 0, fonts, motion, staticA2Blocks }; })()");
  ...
  await sleep(500);
  report.signatureScroll = await evaluate("(() => { const triggers = document.querySelectorAll('.pin-spacer').length ? [1] : []; const fiber = document.querySelector('.hero__fibers'); const hero = document.querySelector('.hero'); const fiberTransform = fiber ? getComputedStyle(fiber).transform : 'none'; return { ok: triggers.length === 1 && Boolean(document.querySelector('.pin-spacer')) && fiberTransform !== 'none' && Boolean(hero), triggerCount: triggers.length, pinSpacer: Boolean(document.querySelector('.pin-spacer')), fiberTransform }; })()");
  if (!report.signatureScroll?.ok) fail("Signature scroll contract failed: " + JSON.stringify(report.signatureScroll));

  const motionRange = await evaluate("(() => { const spacer = document.querySelector('.pin-spacer'); if (!spacer) return null; const start = spacer.getBoundingClientRect().top + window.scrollY; const end = start + Math.max(300, document.querySelector('.manifesto')?.offsetHeight * 1.35 || 300); return { start, end }; })()");
  if (motionRange) {
    const phases = [
      ["rest", 0],
      ["separate", 0.33],
